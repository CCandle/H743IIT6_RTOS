...
{		
	//注意：复位引脚初始化配置已单独完成，此处只需要操作此引脚就OK
...
void System_Check_Temp(void)
{
	...
	do
	{
		...
		{
			vTaskDelay(pdMS_TO_TICKS(100));                  //若MCU 速度太快，必要時使用
			LCD_CmdWrite(0x01);
			vTaskDelay(pdMS_TO_TICKS(100));                  //若MCU 速度太快，必要時使用
			temp =LCD_DataRead();
...
//    unsigned char max_retries = 10; // 最大重试次数
//    unsigned char temp = 0;
////    system_ok = 0;
//    
//    // 先等待一段时间让芯片稳定
...
//			z++;
//        // 检查状态寄存器是否可写
//        if((LCD_StatusRead() & 0x02) == 0x00)    // 检查状态寄存器bit1(忙标志)
//        {
//            vTaskDelay(2); // 必要延时
//            
//            // 读取CCR寄存器(0x01)
...
//        {
//            LT768_HW_Reset(); // 硬件复位
//            vTaskDelay(50); // 复位后等待足够长时间让芯片重新初始化
//            retry_count = 0; // 重置计数器重新开始
//            
//            // 可选：增加最大重试次数避免无限循环
//            if(max_retries < 20) {
//                max_retries += 5;
...
//        }
//        
//    } while(system_ok == 0 && retry_count < 30); // 绝对最大重试次数
//    
//    // 最终状态检查
//    if(system_ok == 0) {
//        // 如果还是失败，可以记录错误或采取其他措施
//        Error_Handler();
//    }
//}
...
//    // 设置绘图命令为画圆
//    LCD_CmdWrite(0x68); // 绘图命令寄存器
//    LCD_DataWrite(0x04); // 画圆命令

//    // 设置圆心坐标和半径
//    LCD_CmdWrite(0x69); // 圆心X坐标低8位
//    LCD_DataWrite(x0 & 0xFF);
//    LCD_CmdWrite(0x6A);
//    LCD_DataWrite((x0 >> 8) & 0xFF);

//    LCD_CmdWrite(0x6B); // 圆心Y坐标低8位
//    LCD_DataWrite(y0 & 0xFF);
//    LCD_CmdWrite(0x6C);
//    LCD_DataWrite((y0 >> 8) & 0xFF);

//    LCD_CmdWrite(0x6D); // 半径低8位
//    LCD_DataWrite(radius & 0xFF);
//    LCD_CmdWrite(0x6E);
//    LCD_DataWrite((radius >> 8) & 0xFF);

//    // 触发绘图命令
//    LCD_CmdWrite(0x67); // 绘图触发寄存器
//    LCD_DataWrite(0x01); // 开始绘图

//    // 等待绘图完成
//    while(LCD_StatusRead() & 0x01); // 等待绘图忙结束
//}
